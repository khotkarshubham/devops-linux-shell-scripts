...
# Backup variables
BACKUP_DIR="/home/username/Backup-Builds/servicedirectory"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
BACKUP_FILE="$BACKUP_DIR/servicedirectory_backup_$TIMESTAMP.tar.gz"
...
if [ "$(ls -A $BACKEND_DIR 2>/dev/null)" ]; then
    echo "Backing up existing build to $BACKUP_FILE..."
    sudo tar -czvf "$BACKUP_FILE" -C "$BACKEND_DIR" . || { echo "Backup failed"; exit 1; }
else
